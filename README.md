...
현재 운영 기준에서 새 UI와 active CRUD는 `frontend/`의 Next.js 앱 안에 있는 Prisma 경로를 기준으로 이해하면 됩니다.

저장소 루트의 Spring Boot + MyBatis 백엔드는 여전히 남아 있지만, 현재 그 역할은 전체 CRUD의 중심이 아니라 예외 경계에 가깝습니다.

이 의미는 다음과 같습니다.

- MyBatis가 아직 제거된 것은 아닙니다.
- active CRUD 기준 Prisma 전환은 사실상 완료된 상태로 봐도 됩니다.
...
- 그래서 이 프로젝트에서는 archive-facing CRUD를 Prisma로 옮기고, Kakao 연동에 직접 맞물린 경계만 Spring/MyBatis에 남기는 쪽이 더 현실적인 선택이었습니다.
