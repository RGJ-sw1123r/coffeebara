...
- `media_asset` + `media_attachment` 구조 기반 이미지 첨부 메타데이터/연결 모델 추가
- `CAFE_RECORD` 기준 로컬 이미지 업로드 API 및 첨부 조회 응답 구조 추가
- 회원 / 게스트 상태 분리
- 기록 카드 정렬 및 삭제 흐름 반영
...
현재 이미지 첨부는 백엔드 기준으로만 연결되어 있습니다.
...
- 첨부 조회는 `media_asset`, `media_attachment` 테이블이 준비되어 있어야 완전 동작합니다.
- 두 테이블이 아직 없으면 기존 기록은 계속 조회되지만, 프론트에서는 이미지 정보 누락 경고 토스트가 표시됩니다.
...
현재는 카페 검색, 장소 캐시, 회원 저장 카페, 회원 텍스트 기록, 게스트 저장 흐름까지 정리된 상태이며, active CRUD는 Prisma 기준으로 전환되었습니다. MyBatis는 Kakao 로그인 시 `app_user` upsert와 `cafe` 마스터 upsert 같은 예외 경계에만 남아 있고, 이후 새 기능과 UI는 Prisma 기준으로 붙여 가는 방향입니다.
