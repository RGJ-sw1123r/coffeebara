...
- `frontend` 앱 기준 Prisma bootstrap 및 DB 연결 검증 완료

다음 정리 대상:

- MyBatis → Prisma 점진 전환 경계 정리
...
현재 운영 기준의 주 DB 접근은 여전히 저장소 루트의 Spring Boot + MyBatis 백엔드에 있습니다.

다만, `frontend/`의 Next.js 앱 안에는 점진 전환을 위한 Prisma bootstrap이 추가되어 있으며, DB 연결 검증까지 완료된 상태입니다.

이 의미는 다음과 같습니다.

- MyBatis가 아직 제거된 것은 아닙니다.
- Prisma는 앞으로 일부 기능을 슬라이스 단위로 옮기기 위한 준비 상태입니다.
- 인증, 보안, OAuth 같은 코어 백엔드 역할은 당분간 Spring 백엔드에 남기는 방향을 우선합니다.
- 데이터 접근 전환은 한 번에 전체를 갈아엎는 방식이 아니라, 안전한 기능 단위로 점진적으로 진행할 예정입니다.
...
- ESLint 9
- Zustand 5
- Prisma ORM (bootstrap completed in `frontend` for gradual migration)
...
지금은 카페 검색, 검색/지도 기반 홈 흐름, 장소 캐시, 회원 저장 카페, 회원 텍스트 기록 흐름, 게스트 저장 흐름까지 정리된 상태이며, Prisma bootstrap과 DB 연결 검증도 마친 상태입니다. 다음 단계는 MyBatis를 무작정 걷어내는 것이 아니라, 실제 기능 슬라이스 단위로 안전하게 Prisma 전환 범위를 넓혀 가는 일입니다.
