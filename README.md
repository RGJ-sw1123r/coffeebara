...
이 프로젝트는 OpenAI Codex CLI를 활용한 vibe coding 실험입니다.

이 프로젝트에서 확인하고자 한 것은 두 가지였습니다.  
첫째, ChatGPT를 통해 이미 경험한 생산성 향상을 넘어, Codex CLI가 실제 개발 흐름 안에서 얼마나 강력하게 작동하는지였습니다.  
둘째, AI가 작성한 코드를 결국 사람이 직접 마무리해야 하는지, 아니면 적절한 지시와 맥락만으로도 원하는 결과를 만들 수 있는지였습니다.

이번 작업에서는 코드를 직접 따라가며 수정하기보다,  
AI에게 어떤 지시를 주어야 원하는 구조와 흐름이 나오는지에 집중했습니다.  
단순한 수정사항조차 곧바로 코드를 손보기보다,  
왜 그런 결과가 나왔는지 먼저 살피고 AI에게 더 정확하게 지시하는 방식을 택했습니다.  
또한 AI가 불필요하게 우회하거나 같은 흐름을 반복하는 지점을 찾아 원인을 파악한 뒤,  
더 효율적인 방법을 다시 지시해 오류를 수정하는 경험도 했습니다.

AGENTS.md는 이 과정에서 사용한 작업 원칙과 제약을 정리한 문서입니다.  
즉, 이 저장소는 결과물이면서 동시에 실험 기록이기도 합니다.
